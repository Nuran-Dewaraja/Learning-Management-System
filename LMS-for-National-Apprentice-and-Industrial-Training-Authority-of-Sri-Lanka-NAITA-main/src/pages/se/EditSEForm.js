...
const EditSEForm = () => {
  ...
  const [title, setTitle] = useState(se.title);
  const [pdfString, setPdfString] = useState(se.pdfString);
  const [description, setDescription] = useState(se.description);
  const [timestamp] = useState(se.timestamp); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:1212/courses/edit-software/${se.id}`, updatedSE);
      navigate('/se-list', { state: { refresh: true } }); 
    } catch (error) {
      console.error('Error updating SE content:', error);
    }
  };
      ...
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          <FilePen className="text-naita me-2"/>
            Edit Software Engineering Content
        </h3>
        <p className="text-secondary mt-1">Edit Software Engineering lecture materials and content</p>
      </div>
