...
const SEForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:1212/courses/software', newSEEntry);
      setId(response.data)
      setMessage(`Software Engineering entry created successfully with ID: ${response.data}`);
      navigate('/se-list', { state: { refresh: true } }); 
    } catch (error) {
      console.error("Error saving SE entry:", error);
      setMessage('Error saving SE entry');
    }
  };
    ...
    <div style={{ maxWidth: '50%', margin: '0 auto', padding: '20px' }}>
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          ...
          Create Software Engineering Content
          </h3>
          <p className="text-secondary mt-1">Create new Software Engineering lecture materials and content</p>
          </div>
