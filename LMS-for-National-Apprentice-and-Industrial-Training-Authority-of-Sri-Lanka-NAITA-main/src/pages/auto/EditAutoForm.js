...
const EditAutoForm = () => {
  ...
  const [pdfString, setPdfString] = useState(auto.pdfString);
  const [description, setDescription] = useState(auto.description);
  const [timestamp] = useState(auto.timestamp); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:1212/courses/edit-automobile/${auto.id}`, updatedAuto);
      navigate('/auto-list', { state: { refresh: true } });
    } catch (error) {
      console.error('Error updating Auto content:', error);
    }
  };
      ...
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          <FilePen className="text-naita me-2"/>
            Edit Automobile Content
        </h3>
        <p className="text-secondary mt-1">Edit Automobile lecture materials and content</p>
      </div>
