...
const ETForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:1212/courses/electrical', newETEntry);
      setId(response.data)
      setMessage(`ET entry created successfully with ID: ${response.data}`);
      navigate('/et-list', { state: { refresh: true } });
    } catch (error) {
      console.error("Error saving ET entry:", error);
      setMessage('Error saving ET entry');
    }
  };
    ...
    <div style={{ maxWidth: '50%', margin: '0 auto', padding: '20px' }}>
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          ...
          Create Electrical Technician Content
          </h3>
          <p className="text-secondary mt-1">Create new Electrical Technician lecture materials and content</p>
          </div>
