...
const EditETForm = () => {
  ...
  const { et } = location.state; 

  const [title, setTitle] = useState(et.title);
  const [pdfString, setPdfString] = useState(et.pdfString);
  const [description, setDescription] = useState(et.description);
  const [timestamp] = useState(et.timestamp);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:1212/courses/edit-electrical/${et.id}`, updatedET);
      navigate('/et-list', { state: { refresh: true } }); 
    } catch (error) {
      console.error('Error updating ET content:', error);
    }
  };
      ...
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          <FilePen className="text-naita me-2"/>
            Edit Electrical Technician Content
        </h3>
        <p className="text-secondary mt-1">Edit Electrical Technician lecture materials and content</p>
      </div>
