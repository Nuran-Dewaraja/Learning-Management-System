...
const HotelPanel = () => {
  ...
  const handleDelete = async (id) => {
    const confirmed = window.confirm('Are you sure you want to delete this Hotel content?');
    if (confirmed) {
      try {
        await axios.delete(`http://localhost:1212/courses/delete-hotel/${id}`);
        window.location.reload();

      } catch (error) {
        console.error('Error deleting Hotel entry:', error);
      }
    }
  };

const handleEdit = (hotel) => {
  navigate('/edit-hotel', { state: { hotel } }); 
};

  return (
    <div style={{ maxWidth: '65%', margin: '0 auto', padding: '15px' }}>
      <div className="d-flex justify-content-between align-items-center m-3">
      <div><h3>Manage Hotel Course Resources</h3>
      <p className="text-secondary">Add, edit, or delete lecture materials, quizzes, assessments, and other course content.</p> </div>
      <button className="btn btn-success mx-3" onClick={() => navigate('/hotel-lecturer')}>Add New Hotel Content</button>
      </div>
