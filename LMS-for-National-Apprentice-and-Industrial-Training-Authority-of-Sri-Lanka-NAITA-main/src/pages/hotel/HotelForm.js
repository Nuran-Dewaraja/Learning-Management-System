...
const HotelForm = () => {
  ...
  const [id, setId] = useState(''); // State to hold the returned document ID
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:1212/courses/hotel', newHotelEntry);
      setId(response.data)
      setMessage(`Hotel entry created successfully with ID: ${response.data}`);
      navigate('/hotel-list', { state: { refresh: true } });
    } catch (error) {
      console.error("Error saving Hotel entry:", error);
      setMessage('Error saving Hotel entry');
    }
  };
    ...
    <div style={{ maxWidth: '50%', margin: '0 auto', padding: '20px' }}>
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          ...
          <p className="text-secondary mt-1">Create new Hotel lecture materials and content</p>
          </div>
