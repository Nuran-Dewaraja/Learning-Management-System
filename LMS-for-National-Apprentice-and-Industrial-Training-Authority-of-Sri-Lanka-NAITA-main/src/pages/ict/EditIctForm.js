...
const EditIctForm = () => {
  ...
  const { ict } = location.state; 

  const [title, setTitle] = useState(ict.title);
  const [pdfString, setPdfString] = useState(ict.pdfString);
  const [description, setDescription] = useState(ict.description);
  const [timestamp] = useState(ict.timestamp); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:1212/courses/edit-ict/${ict.id}`, updatedIct);
      navigate('/ict-list', { state: { refresh: true } }); 
    } catch (error) {
      console.error('Error updating ICT content:', error);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '50%', margin: '0 auto', padding: '20px' }}>
      <div className="my-6">
        ...
        <p className="text-secondary mt-1">Edit ICT lecture materials and content</p>
      </div>
