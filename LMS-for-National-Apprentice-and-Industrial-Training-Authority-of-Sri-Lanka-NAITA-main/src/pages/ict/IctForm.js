...
const IctForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:1212/courses/ict', newIctEntry);
      setId(response.data)
      setMessage(`ICT entry created successfully with ID: ${response.data}`);
      navigate('/ict-list', { state: { refresh: true } });
    } catch (error) {
      console.error("Error saving ICT entry:", error);
      setMessage('Error saving ICT entry');
    }
  };
    ...
    <div style={{ maxWidth: '50%', margin: '0 auto', padding: '20px' }}>
      <div className="my-6">
        <h3 className="d-flex align-items-center">
          ...
          <p className="text-secondary mt-1">Create new ICT lecture materials and content</p>
          </div>
